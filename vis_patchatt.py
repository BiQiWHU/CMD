...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        for i, img in enumerate(images):
            ...
            # img = img.reshape(img.shape[-1], img.shape[-2], img.shape[0]) * 256
            # cv2.imwrite('vis/patch/'+'{}_view.png'.format(i), img)
            img = img[0]
            torchvision.utils.save_image(img, 'vis/patch/'+'{}_view.png'.format(i))
        exit()
    ...
    def forward(self, student_output, teacher_output, epoch, targets_mixup):
        ...
        for iq, q in enumerate(t_cls):
            for v in range(len(s_cls)):
                ...
                # print(t_cls[iq].shape)
                # print(s_cls[v].shape)
                # print(t_region_cur.shape)
                # print(s_region_cur.shape)
                t_region_mil = torch.sum(t_region_cur, dim=1)
                s_region_mil = torch.sum(s_region_cur, dim=1)
                # print(t_region_mil.shape)
                # print(s_region_mil.shape)
                loss_md += F.kl_div(t_cls[iq], t_region_mil)
                loss_md += F.kl_div(s_cls[v], s_region_mil)
                ...
                n_loss_terms += 1
        total_loss /= n_loss_terms
        total_loss += loss_md / n_loss_terms * 0.1
